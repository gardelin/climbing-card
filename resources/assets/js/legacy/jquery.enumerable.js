(function (jQuery) {
    var methods = {
        ...
        select: function (enumerable, callback) {
            var result = [];
            jQuery.each(enumerable, function (index) {
                if (callback.call(this, index)) result.push(this);
            });
            return result;
        },
    ...
    function makeStaticFunction(f) {
        return function () {
            if (arguments.length > 1)
                // The first argument is the enumerable
    ...
    function makeIteratorFunction(staticFunction) {
        return function () {
            ...
            return staticFunction.apply(
                this,
                [this].concat(toArray.apply(arguments)),
            );
        };
    }
