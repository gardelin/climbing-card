...
var Raphael = (function (type) {
    ...
    if (type == 'VML') {
        ...
        var thePath = function (params, pathString, VML) {
            ...
            var el = document.createElement('rvml:shape'),
                ol = el.style
            ...
            p.moveTo = function (x, y) {
                ...
                d +=
                    Math.round(_getX(parseFloat(x, 10))) +
                    ' ' +
                    Math.round(_getY(parseFloat(y, 10)))
                this[0].path = this.Path += d
                this.last.x =
                    (this.isAbsolute ? 0 : this.last.x) +
                    _getX(parseFloat(x, 10))
                this.last.y =
                    (this.isAbsolute ? 0 : this.last.y) +
                    _getY(parseFloat(y, 10))
            ...
            p.arcTo = function (rx, ry, large_arc_flag, sweep_flag, x2, y2) {
                ...
                d += [
                    left,
                    top,
                    Math.round(left + rx * 2),
                    Math.round(top + ry * 2),
                    Math.round(x1),
                    Math.round(y1),
                    Math.round(_getX(parseFloat(x2, 10))),
                    Math.round(_getX(parseFloat(y2, 10))),
                ].join(', ')
                this[0].path = this.Path += d
                this.last.x =
                    (this.isAbsolute ? 0 : this.last.x) +
                    _getX(parseFloat(x2, 10))
                this.last.y =
                    (this.isAbsolute ? 0 : this.last.y) +
                    _getY(parseFloat(y2, 10))
                this.last.isAbsolute = this.isAbsolute
                this.path.push({
                    type: 'arc',
                    arg: [].slice.call(arguments, 0),
            ...
            p.cplineTo = function (x1, y1, w1) {
                if (!w1) {
                    ...
                    var x = Math.round(
                        p._getX(Math.round(parseFloat(x1, 10) * 100) / 100)
                    )
                    var y = Math.round(
                        p._getY(Math.round(parseFloat(y1, 10) * 100) / 100)
                    )
                    var w = Math.round(
                        VML._getW(Math.round(parseFloat(w1, 10) * 100) / 100)
                    )
                    var d = this.isAbsolute ? 'c' : 'v'
                    var attr = [
                        Math.round(this.last.x) + w,
                        Math.round(this.last.y),
                        x - w,
                        y,
                        x,
                        y,
            ...
            p.curveTo = function () {
                ...
                if (arguments.length == 6) {
                    this.last.bx =
                        (this.isAbsolute ? 0 : this.last.x) +
                        _getX(parseFloat(arguments[2], 10))
                    this.last.by =
                        (this.isAbsolute ? 0 : this.last.y) +
                        _getY(parseFloat(arguments[3], 10))
                    this.last.x =
                        (this.isAbsolute ? 0 : this.last.x) +
                        _getX(parseFloat(arguments[4], 10))
                    this.last.y =
                        (this.isAbsolute ? 0 : this.last.y) +
                        _getY(parseFloat(arguments[5], 10))
                    d +=
                ...
                if (arguments.length == 4) {
                    ...
                    this.last.bx =
                        (this.isAbsolute ? 0 : this.last.x) +
                        _getX(parseFloat(arguments[0], 10))
                    this.last.by =
                        (this.isAbsolute ? 0 : this.last.y) +
                        _getY(parseFloat(arguments[1], 10))
                    this.last.x =
                        (this.isAbsolute ? 0 : this.last.x) +
                        _getX(parseFloat(arguments[2], 10))
                    this.last.y =
        ...
        var setFillAndStroke = function (o, params) {
            ...
            if (
                typeof params.opacity != 'undefined' ||
                typeof params['stroke-width'] != 'undefined' ||
                typeof params.fill != 'undefined' ||
                typeof params.stroke != 'undefined'
            ) {
                o = o.shape || o[0]
                var fill =
                    (o.getElementsByTagName('fill') &&
                        o.getElementsByTagName('fill')[0]) ||
                    document.createElement('rvml:fill')
                if ('fill-opacity' in params || 'opacity' in params) {
                    fill.opacity =
                        ((params['fill-opacity'] + 1 || 2) - 1) *
                        ((params.opacity + 1 || 2) - 1)
                }
                if (params.fill) {
                    fill.on = true
                }
                ...
                o.appendChild(fill)
                var stroke =
                    (o.getElementsByTagName('stroke') &&
                        o.getElementsByTagName('stroke')[0]) ||
                    document.createElement('rvml:stroke')
                if (
                ...
                params['stroke-width'] &&
                    (stroke.weight =
                        ((parseFloat(params['stroke-width'], 10) || 1) * 12) /
                        16)
        ...
        var addGrdientFill = function (o, gradient) {
            ...
            if (gradient.dots.length) {
                ...
                fill.method = 'none'
                fill.type =
                    gradient.type.toLowerCase() == 'linear'
                        ? 'gradient'
                        : 'gradientTitle'
                ...
                if (
                    typeof gradient.dots[gradient.dots.length - 1].color !=
                    'undefined'
                ) {
                    fill.color2 =
                        gradient.dots[gradient.dots.length - 1].color || '#000'
                }
                var colors = []
                for (var i = 0, ii = gradient.dots.length; i < ii; i++) {
                    if (gradient.dots[i].offset) {
                        colors.push(
                            gradient.dots[i].offset +
                                ' ' +
                                gradient.dots[i].color
                        )
                    }
                }
                var fillOpacity = gradient.dots[0].opacity || 1
                var fillOpacity2 =
                    gradient.dots[gradient.dots.length - 1].opacity || 1
                if (colors) {
                ...
                if (gradient.vector) {
                    var angle =
                        Math.round(
                            Math.atan(
                                (parseFloat(gradient.vector[3], 10) -
                                    parseFloat(gradient.vector[1], 10)) /
                                    (parseFloat(gradient.vector[2], 10) -
                                        parseFloat(gradient.vector[0], 10))
                            ) * 57.29
                        ) || 0
        ...
        Element.prototype.attr = function () {
            ...
            if (this[0].tagName.toLowerCase() == 'group') {
                ...
                for (var i = 0, ii = children.length; i < ii; i++) {
                    this.attr.apply(
                        new item(children[i], this[0], this.vml),
                        arguments
                    )
                }
            } else {
        ...
        var theText = function (vml, x, y, text) {
            ...
            var el = document.createElement('rvml:shape'),
                ol = el.style
            var path = document.createElement('rvml:path'),
                ps = path.style
            path.v = [
                'm',
                Math.round(x),
        ...
        var theGroup = function (vml) {
            var el = document.createElement('rvml:group'),
                els = el.style
        ...
        r._create = function () {
            ...
            rs.height = height
            r.coordsize =
                (width == '100%' ? width : parseFloat(width)) +
                ' ' +
                (height == '100%' ? height : parseFloat(height))
            r.coordorigin = '0 0'

            var b = document.createElement('rvml:rect'),
                bs = b.style
            bs.left = bs.top = 0
            bs.width = rs.width
            bs.height = rs.height
    ...
    if (type == 'SVG') {
        ...
        var thePath = function (params, pathString, SVG) {
            ...
            p.moveTo = function (x, y) {
                ...
                this.last.x =
                    (this.isAbsolute ? 0 : this.last.x) +
                    SVG._getX(parseFloat(x, 10))
                this.last.y =
                    (this.isAbsolute ? 0 : this.last.y) +
                    SVG._getY(parseFloat(y, 10))
                this.path.push({
                    type: 'move',
                    arg: arguments,
                    pos: this.isAbsolute,
            ...
            p.arcTo = function (rx, ry, large_arc_flag, sweep_flag, x, y) {
                ...
                var _getY = this.isAbsolute ? SVG._getY : SVG._getH
                d += [
                    SVG._getW(parseFloat(rx, 10)),
                    SVG._getH(parseFloat(ry, 10)),
                    0,
                    large_arc_flag,
                    sweep_flag,
                    _getX(parseFloat(x, 10)),
            ...
            p.cplineTo = function (x1, y1, w1) {
                if (!w1) {
                    ...
                    var w = SVG._getW(
                        Math.round(parseFloat(w1, 10) * 100) / 100
                    )
            ...
            p.curveTo = function () {
                ...
                if (arguments.length == 6) {
                    ...
                    for (var i = 0, ii = arguments.length; i < ii; i++) {
                        d +=
                            p[i % 2 == 0 ? '_getX' : '_getY'](
                                Math.round(parseFloat(arguments[i], 10) * 100) /
                                    100
                            ) + ' '
                    }
                    this.last.x =
                        (this.isAbsolute ? 0 : this.last.x) +
                        p._getX((parseFloat(arguments[4], 10) * 100) / 100)
                    this.last.y =
                        (this.isAbsolute ? 0 : this.last.y) +
                        p._getY((parseFloat(arguments[5], 10) * 100) / 100)
                    this.last.bx = p._getX(
                        (parseFloat(arguments[2], 10) * 100) / 100
                    )
                    this.last.by = p._getY(
                        (parseFloat(arguments[3], 10) * 100) / 100
                    )
                } else {
                    if (arguments.length == 4) {
                        ...
                        for (var i = 0, ii = arguments.length; i < ii; i++) {
                            d +=
                                p[i % 2 == 0 ? '_getX' : '_getY'](
                                    (parseFloat(arguments[i], 10) * 100) / 100
                                ) + ' '
                        }
                    }
                    this.last.x =
                        (this.isAbsolute ? 0 : this.last.x) +
                        p._getX((parseFloat(arguments[2], 10) * 100) / 100)
                    this.last.y =
                        (this.isAbsolute ? 0 : this.last.y) +
                        p._getY((parseFloat(arguments[3], 10) * 100) / 100)
                    this.last.bx = p._getX(
                        (parseFloat(arguments[0], 10) * 100) / 100
                    )
                    this.last.by = p._getY(
                        (parseFloat(arguments[1], 10) * 100) / 100
                    )
                }
                var oldD = this[0].getAttribute('d') || ''
                this[0].setAttribute('d', oldD + d)
        ...
        var updatePosition = function (o) {
            if (o.pattern) {
                ...
                o.pattern.setAttribute(
                    'patternTransform',
                    'translate(' + [bbox.x, bbox.y].join(',') + ')'
                )
            }
        }
        var setFillAndStroke = function (o, params) {
            ...
            for (var att in params) {
                ...
                switch (att) {
                    ...
                    case 'text':
                        if (o.type == 'text') {
                            o[0].childNodes.length &&
                                o[0].removeChild(o[0].firstChild)
                    ...
                    case 'fill':
                        ...
                        if (isURL) {
                            ...
                            var ig = document.createElementNS(
                                o.svg.svgns,
                                'image'
                            )
                ...
                if (x != 0 && !(x == 1 && y == 1)) {
                    ...
                    if (!(ScaleX == 1 && ScaleY == 1)) {
                        var bbox = this.getBBox(),
                            dx =
                                bbox.x * (1 - ScaleX) +
                                (bbox.width / 2 - (bbox.width * ScaleX) / 2),
                            dy =
                                bbox.y * (1 - ScaleY) +
                                (bbox.height / 2 - (bbox.height * ScaleY) / 2)
                        this.transformations[2] = new Matrix(
                            ScaleX,
                            0,
                            0,
                            ScaleY,
        ...
        Element.prototype.toBack = function () {
            if (this[0].parentNode.firstChild != this[0]) {
                this[0].parentNode.insertBefore(
                    this[0],
                    this[0].parentNode.firstChild
                )
            }
            return this
        }
    ...
    if (type == 'VML' || type == 'SVG') {
        ...
        C.linerect = function (x, y, w, h, r) {
            if (r && parseInt(r, 10)) {
                return this.path({ stroke: '#000' })
                    .moveTo(x + r, y)
                    .lineTo(x + w - r, y)
                    .addRoundedCorner(r, 'rd')
                    .lineTo(x + w, y + h - r)
                    .addRoundedCorner(r, 'dl')
                    .lineTo(x + r, y + h)
                    .addRoundedCorner(r, 'lu')
                    .lineTo(x, y + r)
                    .addRoundedCorner(r, 'ur')
                    .andClose()
            }
            return this.path({ stroke: '#000' })
                .moveTo(x, y)
        ...
        C.drawGrid = function (x, y, w, h, wv, hv, color) {
            ...
            for (var i = 1; i < wv; i++) {
                p.moveTo(x + i * columnWidth, y).lineTo(
                    x + i * columnWidth,
                    y + h
                )
            }
            ...
            if ('cx' in this.attrs || 'x' in this.attrs) {
                ...
                ;(function () {
                    ...
                    if (time < ms) {
                        ...
                        that.attr(
                            is_round ? { cx: x1, cy: y1 } : { x: x1, y: y1 }
                        )
                        that.animation_in_progress = setTimeout(
                            arguments.callee,
                            1
                        )
                        C.safari()
        ...
        C.pathfinder = function (p, path) {
            ...
            path.replace(
                /([mzlhvcsqta])\s*((-?\d*\.?\d*\s*,?\s*)+)/gi,
                function (a, b, c) {
                    var params = []
                    c.replace(/(-?\d*\.?\d*)\s*,?\s*/gi, function (a, b) {
                        b && params.push(+b)
                    })
                    while (params.length >= commands[b].length) {
                        commands[b].apply(
                            p,
                            params.splice(0, commands[b].length)
                        )
    ...
    !(
        window.SVGPreserveAspectRatio &&
        window.SVGPreserveAspectRatio.SVG_PRESERVEASPECTRATIO_XMINYMIN == 2
    )
        ? 'VML'
        : 'SVG'
)
...
Raphael.getColor = function (value) {
    var start = (arguments.callee.start = arguments.callee.start || {
        h: 0,
        s: 1,
        b: value || 0.75,
    })
